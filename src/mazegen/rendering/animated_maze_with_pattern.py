...
class AnimatedMazeGeneratorWithPattern:
    ...
    def generate(self, algorithm: str = "prim"):
        ...
        # Generate starting from Entry to ensure it's part of the main component
        start_x, start_y = self.entry
        if algorithm.lower() == "prim":
            # Prim's generator needs update to accept start coords, or we set it manually
            # Assuming we will update PrimGenerator to accept start_x, start_y or use .entry/.exit if set
    ...
    def _make_imperfect(self):
        ...
        # A shortcut is a wall between two path cells that are not adjacent in the path
        shortcuts = []
            ...
            for direction, (dx, dy) in {'N': (0, -1), 'E': (1,0), 'S': (0,1), 'W': (-1,0)}.items():
                ...
                if (nx, ny) in path_set:
                    ...
                    try:
                        ...
                        if abs(ni - i) > 1:
                            # This is a potential shortcut if the wall is closed
                            if cell.has_wall(direction):
                                shortcuts.append((cell, self.grid[ny][nx], direction))
                    except ValueError:
                        pass
        ...
        # 4. Guarantee at least one shortcut if possible (this creates the 2nd path)
        if shortcuts:
            to_remove.append(random.choice(shortcuts))
        
        # 5. Add more random loops based on maze size (approx 5% of potential walls)
        count_to_remove = max(1, int(len(all_candidates) * 0.05))
        if all_candidates:
            # Avoid duplicates
            already_removed = set(to_remove)
            remaining_candidates = [c for c in all_candidates if c not in already_removed]
            if remaining_candidates:
                picks = random.sample(remaining_candidates, min(len(remaining_candidates), count_to_remove - len(to_remove)))
    ...
    def render_frame_simple(self, frame_idx):
        ...
        if not self.renderer:
             ...
             class GenWrapper:
                 def __init__(self, grid, width, height, entry, exit_coords, path_cells, color_scheme):
                     self.grid = grid
                     self.width = width
                     self.height = height
                     self.entry = entry
             ...
             self.renderer = FrameRenderer(GenWrapper(self.grid, self.width, self.height, self.entry, self.exit_coords, path_cells, self.color_scheme))
